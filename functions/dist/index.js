...
async function fetchSteamProfile(steamId) {
    try {
        const response = await fetch(`https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v2/?key=${STEAM_API_KEY.value()}&steamids=${steamId}`);
        const data = await response.json();
        return data.response.players[0];
    }
    catch (error) {
        logger.error(`[goorp-backend] [fetchSteamProfile()] [error]: Failed to fetch profile: ${steamId}`, error);
        return null;
    }
}
// ============================================================================
...
// ============================================================================
async function exchangeSteamTicketHandler(request, response) {
    try {
        ...
        const steamResponse = await fetch(`https://api.steampowered.com/ISteamUserAuth/AuthenticateUserTicket/v1/?key=${STEAM_API_KEY.value()}&appid=${STEAM_APP_ID.value()}&ticket=${ticket}`);
        const steamData = await steamResponse.json();
        if (!steamData.response.params.steamid) {
            response.status(400).send({ error: "Invalid ticket" });
        }
        const firebaseToken = await getAuth().createCustomToken(steamData.response.params.steamid);
...
// SYNC PLAYER PROFILE
// ============================================================================
async function syncPlayerProfileHandler(request, response) {
    try {
        logger.debug("Authorization Header:", request.headers.authorization);
        const authHeader = request.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            logger.error('[goorp-backend] [syncPlayerProfile] [error] Invalid "authorization" header');
            response
                .status(401)
        ...
        try {
            ...
            const displayName = allowedUsers.get(steamProfile.steamid) ||
            ...
            logger.error("[goorp-backend] [syncPlayerProfile] Token verification failed:", error);
...
// UPDATE GOLD
// ============================================================================
async function updateGoldHandler(request, response) {
    var _a, _b;
    try {
        const authHeader = request.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            logger.error('[goorp-backend] [updateGold] [error] Invalid "authorization" header');
            response
                .status(401)
                .json({ error: "Unauthorized - provide a bearer token" });
            return;
        ...
        try {
            ...
            const currentGold = (((_b = (_a = playerDoc.data()) === null || _a === void 0 ? void 0 : _a.stats) === null || _b === void 0 ? void 0 : _b.goldEarned) || 0) + amount;
            if (currentGold < 0) {
            ...
            logger.error("[goorp-backend] [updateGold] Token verification failed:", error);
            response.status(401).json({
                error: "Invalid token: " + (error instanceof Error ? error.message : ""),
            });
        }
...
async function endRaidHandler(request, context) {
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "Must be authenticated to end an raid");
    }
    const { uid } = request.auth;
    const db = getFirestore();
    const { outcome, durationSeconds, kills = 0, deaths = 0, goldEarned = 0, } = request.data;
    ...
    batch.update(playerRef, {
        ...
        "stats.extractionsSuccessful": FieldValue.increment(outcome === "extracted" ? 1 : 0),
        "stats.extractionsFailed": FieldValue.increment(outcome !== "extracted" ? 1 : 0),
        "profile.lastLogin": FieldValue.serverTimestamp(),
    });
    await batch.commit();
...
async function uploadPlayerInvHandler(request) {
    if (!request.auth) {
    ...
    // Validate stash
    const stashValidation = validateGridItems(stash, templates, STASH_WIDTH, STASH_HEIGHT, MAX_STASH_ITEMS, "stash");
    if (!stashValidation.valid) {
        throw new HttpsError("invalid-argument", `Stash validation failed: ${stashValidation.error}`);
    }
    // Validate expedition
    const expeditionValidation = validateGridItems(expedition, templates, EXPEDITION_WIDTH, EXPEDITION_HEIGHT, MAX_EXPEDITION_ITEMS, "expedition");
    if (!expeditionValidation.valid) {
        throw new HttpsError("invalid-argument", `Expedition validation failed: ${expeditionValidation.error}`);
    }
    // Validate equipment
    const equipmentValidation = validateEquipment(equipment, templates);
    if (!equipmentValidation.valid) {
        throw new HttpsError("invalid-argument", `Equipment validation failed: ${equipmentValidation.error}`);
    }
    // Sanitize items
...
async function loadItemTemplates(db) {
    const now = Date.now();
...
function validateGridItems(items, templates, gridWidth, gridHeight, maxItems, gridName) {
...
function validateItem(item, templates, depth) {
...
function validateEquipment(equipment, templates) {
...
function validateEquipmentItem(item, templates) {
    if (!item.t || typeof item.t !== "string") {
        return { valid: false, error: "Missing template ID" };
    }
    if (!/^[a-z0-9_]+:[a-z0-9_]+$/.test(item.t)) {
...
function sanitizeItem(item) {
    ...
    if (item.x !== undefined)
        sanitized.x = Math.floor(item.x);
    if (item.y !== undefined)
        sanitized.y = Math.floor(item.y);
    if (item.slot)
        sanitized.slot = item.slot;
    if (item.r === true)
        sanitized.r = true;
...
async function getShopItemsHandler(request, response) {
    const shopItems = [];
...
async function cancelTradingListingHandler(request, response) {
    try {
        const authHeader = request.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            logger.error('[goorp-backend] [cancelTradingListing] [error] Invalid "authorization" header');
            response
                .status(401)
                .json({ error: "Unauthorized - provide a bearer token" });
            return;
        ...
        try {
            ...
            logger.info(`[goorp-backend] [cancelTradingListing] Listing ${listing_id} cancelled by ${uid}`);
            ...
            logger.error("[goorp-backend] [cancelTradingListing] Token verification failed:", error);
            response.status(401).json({
                error: "Invalid token: " + (error instanceof Error ? error.message : ""),
            });
        }
...
async function getTradingListingsHandler(request, response) {
    try {
        const authHeader = request.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            logger.error('[goorp-backend] [getTradingListings] [error] Invalid "authorization" header');
            response
                .status(401)
                .json({ error: "Unauthorized - provide a bearer token" });
            return;
        ...
        try {
            ...
            logger.info(`[goorp-backend] [getTradingListings] Returned ${listings.length} listings`);
            response.status(200).json({ success: true, listings });
        }
        catch (error) {
            logger.error("[goorp-backend] [getTradingListings] Token verification failed:", error);
            response.status(401).json({
                error: "Invalid token: " + (error instanceof Error ? error.message : ""),
            });
        }
...
async function purchaseTradingListingHandler(request, response) {
    try {
        const authHeader = request.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            logger.error('[goorp-backend] [purchaseTradingListing] [error] Invalid "authorization" header');
            response
                .status(401)
                .json({ error: "Unauthorized - provide a bearer token" });
            return;
        ...
        try {
            ...
            await batch.commit();
            logger.info(`[goorp-backend] [purchaseTradingListing] Listing ${listing_id} purchased by ${buyerId} for ${price} gold`);
            response.status(200).json({
                success: true,
                gold: newBuyerGold,
                item_template_id: listingData.item_template_id,
            ...
            logger.error("[goorp-backend] [purchaseTradingListing] Token verification failed:", error);
            response.status(401).json({
                error: "Invalid token: " + (error instanceof Error ? error.message : ""),
            });
        }
...
async function postTradingListingHandler(request, response) {
    try {
        const authHeader = request.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) {
            logger.error('[goorp-backend] [postTradingListing] [error] Invalid "authorization" header');
            response
                .status(401)
                .json({ error: "Unauthorized - provide a bearer token" });
            return;
        ...
        try {
            ...
            try {
                ...
                logger.warn("[goorp-backend] [postTradingListing] Could not fetch Steam profile:", err);
            }
            ...
            logger.info(`[goorp-backend] [postTradingListing] Listing created: ${listingRef.id} by ${uid}`);
            response.status(200).json({ success: true, listing_id: listingRef.id });
        }
        catch (error) {
            logger.error("[goorp-backend] [postTradingListing] Token verification failed:", error);
            response.status(401).json({
                error: "Invalid token: " + (error instanceof Error ? error.message : ""),
            });
        }
